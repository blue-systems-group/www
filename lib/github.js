var async = require('async'),
		path = require('path'),
    _ = require('underscore'),
    git = require('git-rev-sync');

function doGithub(file, filename) {
	file.github_links = {
		'blob': 'https://github.com/blue-systems-group/www/blob/master/src/' + filename,
		'commits': 'https://github.com/blue-systems-group/www/commits/master/src/' + filename,
		'commit_id': git.short(),
		'commit': 'https://github.com/blue-systems-group/www/commit/' + git.long()
	}
	return;
};

function githubFiles(files) {
  return _.pick(files, function(file, filename) {
		return file.doGithub;
  });
}

module.exports = function(config) {
  return function(files, metalsmith, done) {
    async.forEachOf(githubFiles(files), function(file, filename, finished) {
      doGithub(file, filename);
      finished();
    }, function () {
      done();
    });
  }
};

